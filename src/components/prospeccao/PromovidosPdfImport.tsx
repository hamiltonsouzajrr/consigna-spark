// Importação de promovidos via PDF (funcionalidade legada, mantida como aba
// secundária na tela /prospeccao/promovidos). A leitura do PDF acontece no
// navegador (pdfjs + OCR tesseract) e a IA identifica quem foi promovido.
...
import { Award, Upload, Trash2, Plus, Save, FileText, Search, Loader2 } from "lucide-react";
import {
...
function titleCaseCargo(s: string): string {
  ...
  return s.toLocaleLowerCase("pt-BR").split(/\s+/).filter(Boolean)
    .map((w, idx) => (lower.has(w) && idx > 0 ? w : w.charAt(0).toLocaleUpperCase("pt-BR") + w.slice(1)))
    .join(" ");
}
function titleCaseName(s: string): string { return titleCaseCargo(s); }
...
function cleanName(s: string): string {
  let v = s.replace(/^\s*\d+\s*[-–.)]?\s*/, "").replace(/[-–:|,;]+$/, "").replace(HONORIFIC_RE, "").trim();
...
function extractCargo(s: string): string {
  ...
  if (m && typeof m.index === "number") return t.slice(m.index).replace(/[-–:|,;]+$/, "").trim();
  return "";
}
  ...
  for (let p = 1; p <= doc.numPages; p++) {
    ...
    try {
      ...
      for (const y of sortedY) {
        ...
        if (line) { lines.push(line); pageLineCount += 1; }
      }
    } catch (error) {
...
function parseLines(lines: string[]): Draft[] {
  ...
  for (let i = 0; i < usefulLines.length; i++) {
    ...
    let cargo = extractCargo(after) || extractCargo(beforeRaw) || extractCargo(nextRaw);
    if (!cargo) {
...
export function PromovidosPdfImport() {
  ...
  const handleSave = async () => {
    ...
    if (clean.length === 0) { toast.warning("Preencha nome, CPF e cargo de pelo menos um registro."); return; }
    const invalid = clean.filter((d) => !isValidCpf(d.cpf));
    if (invalid.length > 0) { toast.error(`${invalid.length} CPF(s) inválido(s). Corrija os campos destacados antes de salvar.`); return; }
    ...
    if (deduped.length === 0) { toast.warning("Todos os CPFs já estão cadastrados."); return; }
    setSaving(true);
    try {
      const { inserted } = await saveFn({ data: { mes_referencia: mes, entries: deduped } });
      toast.success(`${inserted} promovido(s) salvo(s).${skipped > 0 ? ` ${skipped} duplicado(s) ignorado(s).` : ""}`);
      setDrafts([]);
      setRawLines([]);
      load();
    } catch (e: any) {
  ...
  const visible = useMemo(() => {
    ...
    return list.filter((p) => {
      ...
      if (term && !(p.nome.toLowerCase().includes(term) || p.cpf.includes(term) || p.cargo.toLowerCase().includes(term))) return false;
      return true;
    });
    ...
    <>
      {isAdmin && (
        <Card className="mb-6 p-4">
          <div className="mb-4 flex flex-wrap items-end gap-3">
            ...
            <div>
              <input ref={fileRef} type="file" accept="application/pdf" className="hidden" onChange={(e) => onPickFile(e.target.files?.[0])} />
          ...
          {rawLines.length > 0 && (
            <details className="mt-4 rounded-md border bg-muted/30 p-3 text-sm">
              ...
              <pre className="mt-3 max-h-64 overflow-auto whitespace-pre-wrap text-xs text-muted-foreground">{rawLines.join("\n")}</pre>
            </details>
          )}
      ...
      <div className="mb-4 flex flex-wrap items-end gap-3">
        ...
        <div>
          ...
          <select value={cargoFilter} onChange={(e) => setCargoFilter(e.target.value)} className="h-9 rounded-md border bg-background px-2 text-sm">
        ...
        <div>
          ...
          <select value={mesFilter} onChange={(e) => setMesFilter(e.target.value)} className="h-9 rounded-md border bg-background px-2 text-sm capitalize">
            <option value="todos">Todos os meses</option>
            {mesOptions.map((m) => <option key={m} value={m}>{fmtMonth(m)}</option>)}
          </select>
        </div>
        {(cargoFilter !== "todos" || mesFilter !== "todos") && (
          <Button size="sm" variant="ghost" onClick={() => { setCargoFilter("todos"); setMesFilter("todos"); }}>Limpar</Button>
        )}
      </div>

      {loadingList && <p className="text-sm text-muted-foreground">Carregando…</p>}
      {!loadingList && grouped.length === 0 && (
        <Card className="p-8 text-center text-sm text-muted-foreground">Nenhum promovido cadastrado ainda.</Card>
      )}
