        ...
        <Section icon={<Phone className="h-4 w-4" />} title={`Telefones (${c.TELEFONES?.length ?? 0})`}>
          ...
          <div className="space-y-2">
            {(c.TELEFONES ?? []).map((t, i) => {
              ...
              const isCelular =
                (t.TIPO_TELEFONE ?? "").toLowerCase().includes("celular") ||
                (t.TIPO_TELEFONE ?? "").toLowerCase().includes("movel") ||
                (t.TIPO_TELEFONE ?? "").toLowerCase().includes("móvel") ||
                (t.TIPO ?? "").toLowerCase().includes("celular");
              return (
                <div key={i} className="flex items-center justify-between rounded-md border border-slate-200 bg-slate-50 p-2.5 text-sm">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {isCelular && (
                        <Badge className="bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100 text-[10px] font-normal px-1.5 py-0">
                          <svg className="w-3 h-3 mr-0.5 inline" viewBox="0 0 24 24" fill="currentColor">
