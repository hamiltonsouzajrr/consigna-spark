...
const DEDUP_HORAS = 3; // não repete o mesmo lembrete dentro desse intervalo
...
export async function dispararLembretesFollowup(): Promise<{
  ...
  const porConsultora = new Map<string, { total: number; proximo: string; nome: string }>();
  for (const l of data ?? []) {
    ...
    if (!atual) {
      porConsultora.set(uid, { total: 1, proximo: quando, nome: (l as any).nome });
    } else {
      atual.total += 1;
  ...
  const jaAvisados = new Set((recentes ?? []).map((r: any) => r.user_id as string));

  const rows = [...porConsultora.entries()]
    .filter(([uid]) => !jaAvisados.has(uid))
    .map(([uid, info]) => ({
      user_id: uid,
      title: "Follow-ups para agora",
      body:
        info.total === 1
          ? `Retorno agendado com ${info.nome}. Abra a aba Follow-ups e faça o contato.`
          : `Você tem ${info.total} follow-ups para agora (o mais urgente: ${info.nome}).`,
    }));
