...
BEGIN
  ...
  -- consultoras ativas com conta no sistema + desempenho na janela
  ...
  -- peso 1..(_peso_max) proporcional ao desempenho relativo ao melhor
  UPDATE _perf p
  ...
  -- leads elegíveis: nunca abordados (histórico preservado)
  CREATE TEMP TABLE _pool ON COMMIT DROP AS
  SELECT d.id,
         row_number() OVER (ORDER BY d.data_publicacao DESC NULLS LAST, d.created_at DESC) AS rn
  FROM public.do_registros d
  WHERE coalesce(d.status_abordagem, 'novo') = 'novo'
    AND d.contatado_em IS NULL
...
REVOKE ALL ON FUNCTION public.redistribuir_do_registros_por_desempenho(integer, integer, numeric) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.redistribuir_do_registros_por_desempenho(integer, integer, numeric) TO service_role;